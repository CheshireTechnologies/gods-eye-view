export const ACTION_DESCRIPTIONS = {
  ...
  analyst_query: {
    description:
      'Answer questions ABOUT the data currently loaded on the map — counts, lists, superlatives, and attribute filters over live layers (flights, military, ships, fires, earthquakes). Examples: "how many flights over Texas", "biggest fire near LA", "which ships are headed to Oakland", "anything above 40,000 feet", "fastest thing in view". Queries ONLY client-side data from ENABLED layers — if the needed layer is off, say so and offer to enable it. For a follow-up about the previous answer\'s set ("which of those is closest?"), set followUp=true and send only the new filters/sort. For "nearest flight from <origin airport>" in view with nothing found, the app may itself pull the camera back and retry, or fall back to the last-known sighting of a match — the result then carries zoomRetried and/or usedHistoricalFallback, and any historical item carries staleSecondsAgo. Say so plainly when either is set ("I widened the view..." / "this is a last-known position from N seconds ago, not live") — never present a historical item as a current one.',
    $position: 1,
    parameters: {
      properties: {
        layers: {
      ...
      properties: {
        ...
        scope: {
          ...
          properties: {
            ...
            km: { description: 'For kind=radius.' },
          },
        },
        limit: {
