...
## Anonymous vehicle tracking labels

- On-screen `VEH-xxxx` traffic detection labels now derive their suffix from each dot's rotating, short-TTL anonymous session token (see `src/layers/traffic/anonymousVehicleTracking.js`) instead of a raw array index, so they age out and reshuffle instead of staying pinned to one dot indefinitely. Detection-box tracking continuity is kept on a separate, non-displayed per-dot handle so a token rotation updates the label text without the box fading out and back in.
- Fixed a crash: disabling the traffic layer called a method that didn't exist on the anonymous-tracking part, throwing on every `disable()`.
