...
- **Scene context** (`get_entity_context`): selected entity from the context store, or visible entities ranked by distance to the view target (≤100km altitude); plus basemap context — view-target picking, 7-point viewport sampling, view-scale classification (global/continental/regional/metro/city/local), reverse geocoding (center ≤750km, viewport samples ≤3,000km), Google Places Nearby via `/api/google/nearby-places` (≤25km), known-landmark matching against `CITY_POIS`, coarse country inference fallback. Context assembly is capped at 1.5s with cache fallbacks; caches are deduped in-flight.
  ...
  - **Tracking start point + time (2026-09)**: a successful `track_entity` records where and when that tracking session began (`_trackingStarts`, a `Map<layerId, record>` in `src/voice/gevActions.js`) — layer, kind, id, label, start lat/lon/altitude, start timestamp. One record PER LAYER FAMILY, so tracking a vessel does not erase an aircraft's start-point data (and vice versa). `get_current_view_state`'s `tracked[]` entries carry a `since` block computed against the matching layer's record: `elapsedSeconds`, `distanceFromStartKm` (great-circle from the start point to the entity's CURRENT position, via the same `haversineKm` used elsewhere in this file), and `averageSpeedKmh` (`null` until `elapsedSeconds >= 5` — a few seconds of dead-reckoned motion is too noisy to report as a rate). A new successful `track_entity` for a layer replaces only that layer's record (never extended); `stop_tracking` clears the whole map unconditionally, even on a partial release failure. Fires are out of scope — `track_entity`'s fire branch flies the camera once rather than establishing continuous tracking, so there is no "current position" to measure distance against.
    - **Known limit — not true concurrent tracking.** The layers themselves still enforce single active camera-follow: taking camera ownership for a new family calls that family's `stopTracking()` on the PREVIOUS owner (`_releaseFollowCamera` → `releaseCameraOwnership` in `src/layers/awareness/controls.js`, unconditionally for aircraft), so tracking a vessel after an aircraft stops the aircraft's tracking too — its `since` data goes stale (harmless: `collectTrackedEntities` only surfaces a family whose `getTrackedInfo()`/`getSelectedInfo()` still returns non-null). The per-family Map avoids compounding that with cross-family data loss, but does not itself deliver "track an aircraft and a vessel at once, both live." Doing that would mean changing the release path to not stop the previous family's tracking on a camera-ownership handoff — a bigger, riskier change to camera-ownership code with extensive existing invariants, not undertaken here.
...
### Semantic query — local Ollama (2026-09)

- Voice tool `semantic_query` answers fuzzy/descriptive questions over live layer data that `analyst_query`'s exact filters cannot express ("ships behaving oddly near the coast"). It shares `analyst_query`'s record gathering and spatial scoping (`src/data/analystEngine.js`'s `createAnalystEngine`) via `src/data/semanticEngine.js`'s `createSemanticEngine`, so the two tools never disagree about what "in view" or "near X" means — only the ranking pass differs.
- Ranking runs client-side: record descriptions + the operator's query are batched to `/api/ollama/embeddings` (Ollama's `/api/embed`, model `OLLAMA_EMBED_MODEL`, default `nomic-embed-text`), ranked by cosine similarity, and the top matches are sent to `/api/ollama/narrate` (Ollama's OpenAI-compatible chat endpoint, model `OLLAMA_CHAT_MODEL`, default `qwen2.5:3b-instruct`) for one short generated narrative.
- Both endpoints are server-side proxies (`server/providers/ollama/`) to `OLLAMA_BASE_URL` (default `http://127.0.0.1:11434`) — the browser never calls Ollama directly (its default CORS policy blocks that), matching this app's existing "browser never calls a local/third-party service directly" architecture.
- Caching follows the Overpass/military-installations idiom: memory `Map` → in-flight coalescing → sha1-keyed disk JSON under `.gev-cache/ollama/{embeddings,narratives}/` (`server/providers/ollama/cache.js`). Embeddings cache for 10 minutes in memory / 7 days on disk (record text repeats often for slow-moving layers); narratives cache for 25 seconds only, since live positions move.
- **Shared cache root (2026-09)**: every provider's on-disk cache (Overpass, military installations, terrain, FIRMS, TomTom, CelesTrak, Launch Library, adsbdb, Ollama) resolves its directory through `server/providers/common/cache-root.js`'s `gevCacheDir()`, instead of each hardcoding `path.join(process.cwd(), '.gev-cache', ...)`. Default is unchanged (`.gev-cache` inside the checkout); setting `GEV_CACHE_DIR` redirects all of it at once — e.g. to an external drive for faster/larger persistent caching that survives a reinstall. Resolved once per process (matching how these paths were already computed once at module/factory load), and falls back to the default automatically if the configured path isn't reachable when the server starts (its own path, or its nearest existing ancestor, must exist) — a one-time console warning, not a crash, and not a retry mid-session if a drive is later unplugged.
- **Degradation**: entirely optional and additive. No API key, no cost, and nothing here changes `analyst_query`, `get_entity_context`, or OpenAI Realtime voice control. When Ollama isn't running or the configured model isn't pulled, `semantic_query` returns `ok:false` with a plain reason; the instructions tell the model to fall back to `analyst_query` or a normal answer rather than claim an answer that didn't come back. `npm run doctor` reports reachability and which of the two models are present.
